    ...
    const fetchTracks = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            data?.message ||
              `Failed to load recently played tracks (${res.status})`,
          );
        }
      ...
      {tracks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:gap-4">
          {tracks.map((track, index) => (
            <div
              key={index}
              className="flex flex-col gap-3 rounded-[22px] border border-white/10 bg-[#16131b] p-4 sm:flex-row sm:items-center"
            >
              <img
